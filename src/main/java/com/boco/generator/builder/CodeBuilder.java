...
import com.boco.generator.model.DataModel;
...
public class CodeBuilder {
	...
	public  static String createTableForOracle(Map<String,List<DataModel>> excelData){
		List<String> listOfType = new ArrayList<>(15);// 数据类型，检验时使用
		//产品不提供的数据类型
		listOfType.add("数字");
		listOfType.add("日期");
		listOfType.add("字符串");
		...
		listOfType.add("其他");
		//mysql数据库常用类型
		listOfType.add("number");
		listOfType.add("integer");
		listOfType.add("date");
		listOfType.add("char");
		listOfType.add("nchar");
		listOfType.add("varchar2");
		listOfType.add("nvarchar2");
		listOfType.add("date");
		listOfType.add("timestamp");
		listOfType.add("long");
		listOfType.add("raw");
		listOfType.add("long raw");
		listOfType.add("blob");
		listOfType.add("clob");
		listOfType.add("nclob");
		listOfType.add("text");
		listOfType.add("longtext");
		listOfType.add("decimal");
		...
		for (Map.Entry<String, List<DataModel>> entry : excelData.entrySet()) {
			...
			for (DataModel dataModel : listOfData) {
				...
				if (listOfType.contains(type)) {
					if ((!"decimal".equals(type)) && (!"数字".equals(type))) {
						if (StringUtil.isNonnegativeInteger(lengthStr)) {
							length = lengthStr;
						} else {
						...
						if (StringUtil.validate(lengthStr, pattern)) {
					...
					throw new RuntimeException("表[" + tableName + "]中[" + dataModel.getChineseName()
							+ "]字段类型不符合模板,字段类型只能为:"+listOfType.toString());
				}
				...
				if (primaryKey) {
					...
					if (typeIsNotEmpty && ("varchar".equals(type) || "string".equals(type))) {
						...
						// 上面提供数据库标准类型,下面代码段用于匹配中文规则属于亿阳产品专供字段
					} else if (typeIsNotEmpty && ("字符串".equals(type))) {
					...
					} else if (typeIsNotEmpty && "整型".equals(type)) {
						if (isAllowEmpty) {
							builder.append("  ").append(fieldName).append(" int (").append(length).append(")");
							builder.append(" default null comment '").append(comment).append("',\n");
						} else {
					...
					} else if (typeIsNotEmpty && "字符".equals(type)) {
						if (isAllowEmpty) {
							builder.append("  ").append(fieldName).append(" char(").append(length).append(")");
							builder.append(" default null comment '").append(comment).append("',\n");
						} else {
					...
					} else if (typeIsNotEmpty && "数字".equals(type)) {
						if (isAllowEmpty) {
							builder.append("  ").append(fieldName).append(" decimal(").append(length).append(",");
							builder.append(precision).append(")").append(" default null comment '").append(comment)
									.append("',\n");
					...
					} else if (typeIsNotEmpty && ("日期".equals(type))) {
						int intLength = Integer.valueOf(length);
						if (intLength <= 8) {
							if (isAllowEmpty) {
								builder.append("  ").append(fieldName).append(" date");
					...
					} else if (typeIsNotEmpty && "其他".equals(type)) {
						if (isAllowEmpty) {
							builder.append("  ").append(fieldName).append(" varchar(").append(64).append(")");
							builder.append(" default null comment '").append(comment).append("',\n");
						} else {
							builder.append("  ").append(fieldName).append(" varchar(").append(64).append(")");
							builder.append(" not null comment '").append(comment).append("',\n");
	 ...
	 * @return
	 */
	public static String createTable(Map<String, List<DataModel>> excelData) {
		List<String> listOfType = new ArrayList<>(15);// 数据类型，检验时使用
		//产品不提供的数据类型
		listOfType.add("数字");
		listOfType.add("日期");
		...
		//mysql数据库常用类型
		listOfType.add("int");
		listOfType.add("bigint");
		listOfType.add("datetime");
		listOfType.add("char");
		listOfType.add("varchar");
		listOfType.add("date");
		listOfType.add("text");
		listOfType.add("longtext");
		listOfType.add("decimal");
